...
function Flavor() {
    ...
    const handlePrev = () => {
        setIndex((prevIndex) => (prevIndex === 0 ? cakeData.length - 1 : prevIndex - 1));
    }

    const handleNext = () => {
        setIndex((prevIndex) => (prevIndex === cakeData.length - 1 ? 0 : prevIndex + 1));
    }
    ...
    return (
        <div style={{ backgroundColor:'#C8A799', height:'852px', width:'393px', backgroundImage:'url(../public/cake.png)', backgroundRepeat:'no-repeat', backgroundPosition:'center'}}>
            ...
            <div style={{backgroundColor:'#FADCCB', margin:'0px 30px', borderRadius:'17px', padding:'5px 20px', height:'500px'}}>
                <p style={{color:'#4A3C3C', fontFamily:'Gluten', fontSize:'20px', fontWeight:'bolder', margin:'15px 10px 0 0'}}>Step 2 of making a cake</p>
                <hr style={{border:'1px solid #4A3C3C'}} />
                <div style={{color:'#000000', marginTop:'20px', marginLeft:'10px'}}>케이크는 무슨 맛으로 할까요?</div>
                <div style={{marginTop:'70px'}}>
                    <button onClick={handlePrev} style={{background:'none', fontSize:'20px', paddingLeft:'5px', color:'#584949', outline:'none'}}>◀</button>
                    <img src={cakeData[index].img_path} alt={cakeData[index].flavor} style={{width:'190px', height:'auto'}} />
                    <button onClick={handleNext} style={{background:'none', fontSize:'20px', paddingRight:'5px', color:'#584949', outline:'none'}}>▶</button>
                </div>
                <p style={{textAlign:'center', color:'black', fontSize:'20px', fontWeight:'bold'}}>{cakeData[index].flavor}</p>
            </div>
